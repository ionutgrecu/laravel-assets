...
class Manager {
    ...
    public function css($attributes = null) {
        ...
        $assets = ($this->pipeline) ? [$this->cssPipeline()] : array_map(function ($asset) {
            $assetVersion = config('assets.asset_version');
...
//<noscript><link rel="stylesheet" href="' . $asset . '"></noscript>' . "\n";
    ...
    public function js($attributes = null): string {
        ...
        $assets = ($this->pipeline) ? [$this->jsPipeline()] : array_map(function ($asset) {
            $assetVersion = config('assets.asset_version');
    ...
    function preload() {
        ...
        $output = '';
        foreach ($this->preload as $asset) {
            $mime        = $this->filenameToMime($asset);
            $as          = explode('/', $mime)[0];
            $priority    = strpos($as, 'image') !== false ? 'priority="high"' : '';
            $crossOrigin = ('image' === $as) ? '' : ' crossorigin="anonymous"';

            $output .= '<link rel="preload" href="' . $asset . '" as="' . $as . '" type="' . $mime . '" ' . $priority . ' ' . $crossOrigin . '">';
        }
    ...
    protected function packLinks(array $links, Closure $minifier) {
        ...
        foreach ($links as $link) {
            ...
            $path = pathinfo($originalLink);
            if (!empty($path['extension']) && 'css' === $path['extension']) {
                $dirArr = explode('/', $path['dirname']);

                $content = str_replace('../../', implode('/', array_slice($dirArr, 0, -2)) . '/', $content);
                $content = str_replace('../', implode('/', array_slice($dirArr, 0, -1)) . '/', $content);
                $content = str_replace('./', implode('/', $dirArr) . '/', $content);

                $content = preg_replace('/font\-display:\s*block/', 'font-display:swap', $content);
            }
    ...
    function filenameToMime(string $filename): string {
